...
| 项 | 值 |
|---|---|
| 版本 | v0.3(与需求 v0.3 同步稿) |
| 上游 | M0 需求分析 docs/requirements-analysis.md v0.3(严谨化评审,2026)|
| 阶段 | M1 架构设计 ✅ 通过 |
| 状态 | ✅ ADR-01~12 全部确认(2026),进入 M2 |
...
> **与需求 v0.3 的对齐要点(2026 严谨化评审 D10–D12 / RPO 口径)**:
> ① 用户**默认仅管理员创建**,开放注册开关默认关闭(D12,落实见 §6.1);
> ② 手机照片**不做后台自动上传**,经 Web 批量上传入库,桌面 Agent 为自动通道(D10,落实见 §6.5 / §6.8);
> ③ **at-rest 加密由部署层磁盘/卷加密承担**,系统内不实现应用层加密(D11,见 §9);
> ④ RPO 双口径:设备→NAS 上传**尽力即时**、NAS 出站备份**默认日级可配**(NFR-REL-05,见 §10.2);
> ⑤ **版本化写入为数据安全底线**(FR-FS-04,与 §5.3 版本模型一致)。
...
- 用户创建:**默认仅管理员创建**(无开放注册页);可选开放注册开关默认关闭,开启后受 FR-NET-04 暴露面基线约束(与需求 D12 对齐)。
...
### 6.5 媒体入库与时间线
1. 入库来源:桌面 Agent 自动上传完成事件、**Web 批量上传**(多选,手机/其他设备)、手动"导入目录" → job 按扩展名白名单扫描(与需求 D10 一致:手机不做后台自动上传);
...
- 平台与边界:Agent 覆盖 **macOS + Windows**(需求 D8);**手机不设 Agent / 不做后台自动上传**,照片经 Web 批量上传入库(需求 D10)。
...
- at-rest(与需求 D11 对齐):数据静态加密由**部署层磁盘/卷加密**承担,系统内不实现应用层加密;系统只保证传输加密与密钥/口令哈希的安全存储;
...
- 备份:数据目录 + PostgreSQL 逻辑导出 → 外接盘/第二台机器脚本(FR-BACKUP-05,M4);RPO 口径:设备→NAS 上传**尽力即时**,NAS 出站备份**默认日级可配**(与需求 NFR-REL-05 一致)。
